...
const LinkCard: React.FC<LinkCardProps> = (item) => {
  return (
    <a
      href={item.link}
      target="_blank"
      className="flex group items-center w-full p-4 transition-all duration-300 rounded-lg backdrop-blur-md bg-white/10 shadow-md hover:bg-white/20 hover:shadow-lg hover:transform hover:scale-105"
    >
      {item.icon && (
      ...
      <div className="flex-1">
        <h3 className="font-mediumhover:text-white text-gray-100">
          {item.title}
        </h3>
        {item.description && (
          <p className="text-xs transition-all duration-300 hover:text-white text-gray-300">
            {item.description}
          </p>
        )}
      </div>
